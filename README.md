...
🚀 Quick Start · 🧠 Architecture · 📖 API Reference · 🎬 Usage Guide · 🗺️ Roadmap · 🤝 Contributing
</div>
<details>
...
Usage Guide
Mode 1: Single Image to Video
Mode 2: Cinematic Storyboard Video
...
ffmpeg libx264\nAAC audio· H.264
MP4Output\n/content/outputs/
GIF Exporter\ndiffusersexport_to_gif
GIFOutput\n/content/user_gallery/
🔊  Audio Synthesis

gTTS Voiceover\nper-scenenarration\n12s clips · fadein/out
Pydub Mixer\nNormalize ·Gain +5dB
BenSound Music\n5 moods ·192k MP3
CompositeAudioClip\nVoice×1.5 · Music ×0.4
...
base_caption (max_length=50)
Augment → 5 motion variants
Select prompt + settings
image, prompt, neg_prompt
steps, guidance, generator(seed)
frames[0] (List[PIL])
frames + effect_type + intensity
processed_frames (List[PIL])
export_to_gif(frames, path)
frames_to_mp4(frames, path, fps=15)
Write /content/outputs/i2v_output_{uid}_{ts}.{ext}
Mirror to /content/user_gallery/
...
preload_model() — skipped if model_loaded == True
parse_storyboard(storyboard_file.name) → scenes, narration
generate_voiceover(scenes) → voiceover_path
generate_background_music(mood) → music_path
generate_gif_frames(prompt, ...) × 5 scenes → frames[]
create_scene_gif(frames, idx) × 5 → gif_paths[]
concatenate_gifs_to_video(gif_paths) → video_path
add_audio_to_video(video, voiceover, music) → final_output
Cleanup temp files
apply_video_effects()
Python
...
⚠️ Known Limitations
No temporal consistency in SDXL frames — Each frame is generated independently; there is no cross-frame attention. This produces a "stop-motion" rather than smooth video aesthetic. This is by design for the cinematic GIF style.
I2VGenXL frame count is fixed — The model produces a fixed-length frame sequence. There is no native control over output duration.
BenSound music dependency — Pipeline B background music requires an active internet connection and depends on BenSound CDN availability. Graceful fallback to silent audio is implemented.
Voiceover language — gTTS is configured to English (lang='en'). Multi-language support requires changing the lang parameter and testing TTS quality per language.
Sequential SDXL inference — Frames are generated one-at-a-time in a Python loop. A batched inference approach (generating all 12 frames in one .pipeline() call with a batched prompt list) could reduce Pipeline B runtime by ~30%.
No streaming output — Video is not streamed; the full file must complete before the Gradio UI updates. Long sessions (>10 min) may time out on free Colab tiers.
Batch mode is CPU-bound between images — The clear_cuda_memory() call between batch images adds ~2–3 seconds overhead per image due to GC.
📋 Changelog
plain
[v2.0.0] — 2025-06-xx
...
Table
Resource	Used For
ali-vilab/i2vgen-xl	Image-to-video synthesis
stabilityai/stable-diffusion-xl-base-1.0	Cinematic frame generation
microsoft/git-base-coco	Auto image captioning
🤗 Diffusers	Diffusion pipeline orchestration
Gradio	Interactive web UI
MoviePy	Video assembly and codec handling
OpenCV	Frame-level post-processing effects
BenSound	Royalty-free background music
gTTS	Text-to-speech voiceover
Pydub	Audio mixing and normalization
